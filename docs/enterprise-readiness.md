...
| --- | --- | --- |
| `UPDATE t SET body = 'x'` | 19,625 ok / 20,250 refused | 16,812 ok / 17,531 refused |
| `INSERT INTO t (body) SELECT body FROM t` | 17,125 ok / 17,750 refused | 1,765 ok / 1,820 refused |
| `DELETE FROM t` | 72,500 ok / 74,687 refused | 3,000 ok |
| buffered `INSERT`s inside `BEGIN`..`COMMIT` | refused at 11,340 | refused at 884 |

Format version 6 moved four of those cells and deliberately left two alone
(AHL-564). A commit record now elides each page's zero hole, so what has to fit
a region is the *used* bytes of the pages a statement writes rather than a count
of whole pages. `UPDATE` on 512-byte rows is the extreme case — it rewrites fat
rows to the eight-byte literal `'x'`, and the pages it produces are ~97% hole,
so its ceiling went from 1,687 rows to 16,812. `INSERT ... SELECT` at the same
width barely moved (1,687 -> 1,765) because it writes 512-byte rows back into
densely packed pages, which is the same result read from the other side: the
gain is exactly the hole, and a full page has none.

The **buffered-`INSERT` row did not move at all**, and that is by construction
rather than by luck. `Storage::transaction_is_nearly_full` is answered from
`CowBTree::pending_record_len`, which stayed an upper bound computed from whole
pages — finding each page's hole would mean re-scanning the whole dirty set
before every statement, and bounding upward keeps this refusal at exactly the
point it always had. Only `commit`, measuring the record it actually encoded,
