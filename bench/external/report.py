...
def print_server_oltp(directory: str, results: list[dict]) -> None:
    ...
    print("one password as configured here, but InlaySQL has no user table, no grants and no")
    print("per-table permissions at all, a capability gap this benchmark does not exercise either")
    print("way. Neither side negotiates TLS here, but only MySQL's could: InlaySQL's wire protocol")
    print("does not implement it yet. PostgreSQL has no row here on purpose — InlaySQL has no")
