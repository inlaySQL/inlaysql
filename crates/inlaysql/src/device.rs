...
/// Read once per process, like [`shared_read_cache_budget`].
...
/// Read once per process, like [`gate_phases_enabled`].
...
mod group_commit_tests {
    ...
    fn test_coordinator(name: &str) -> CommitCoordinator {
        ...
        CommitCoordinator {
            ...
            layout_boundary: AtomicUsize::new(0),
            gate_phase_ns: [const { AtomicU64::new(0) }; GATE_PHASES],
            // Never from the ambient environment: a diagnostic switch has
            // to mean the same thing in a test whatever the shell exported.
