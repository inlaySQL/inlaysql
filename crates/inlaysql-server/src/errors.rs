...
pub fn from_engine(error: &Error) -> MysqlError {
    match error {
        ...
        // [`Error::Transaction`] is the engine's one variant for every
        // transaction-*state* refusal — "already open", "no transaction open",
        // "cannot CREATE INDEX inside a transaction" — plus the one that is
        // not a state problem at all: a buffered transaction that grew past
        // what one write-ahead-log region can commit. Lumping that in with
        // the others under a flat `1568` used to bury it exactly the way a
        // generic `1030` buried the single-statement version of the same
        // refusal (see `classify_storage`); `classify_transaction` pulls it
        // out by message, the same way `classify_catalog` and
        // `classify_constraint` pull their own distinct conditions out of one
        // engine variant.
        Error::Transaction(message) => classify_transaction(message),
...
/// failure, and a transaction record that outgrew the write-ahead log is not
/// reported as an anonymous one either. The read-only refusal is generated by
/// `Database::check_writable`, which is the planning-based check — this only
/// classifies what it already decided. The size refusal is generated at
/// commit, in `CowBTree::commit` (`btree/tree.rs`), for exactly the case
/// [`classify_transaction`] catches earlier in an explicit transaction: a
/// single autocommit statement (or the final flush of a transaction) whose
/// buffered writes turned out too large to fit in one record. Before this it
/// fell through to the line below and reached the client as `1030` — a
/// storage engine error, naming nothing, retryable-looking, and it is what the
/// `ann-benchmarks` bulk-load run actually hit (`bench/README.md`).
fn classify_storage(message: &str) -> MysqlError {
    ...
    if message.contains("does not fit the write-ahead log") {
        return transaction_too_large(message);
    }
    MysqlError::new(
        1030,
        "HY000",
...
/// Split [`Error::Transaction`] so the one condition that is a size problem —
/// not a state problem — gets a code that says so, instead of the flat
/// `1568` every other message here still gets. See [`classify_storage`] for
/// the sibling refusal this pairs with: same limit, caught at a different
/// moment (mid-transaction here, at commit there), and previously two
/// different codes — `1568` and `1030` — for what is the same fact told to the
/// client twice, inconsistently.
fn classify_transaction(message: &str) -> MysqlError {
    if message.contains("too large for the write-ahead log") {
        return transaction_too_large(message);
    }
    MysqlError::new(1568, "25001", format!("Transaction error: {message}"))
}

/// `ER_TRANS_CACHE_FULL` (1197) — a transaction whose commit record does not
/// fit the fixed-size write-ahead-log region this storage backend commits
/// into.
///
/// MySQL has two errors that could name this: `1153`
/// (`ER_NET_PACKET_TOO_LARGE`), raised when a *wire packet* exceeds
/// `max_allowed_packet`, and `1197` (`ER_TRANS_CACHE_FULL`), raised when a
/// *transaction* exceeds `max_binlog_cache_size`. This is the second shape,
/// not the first: nothing here is refusing to *read* a packet off the socket
/// — every byte of the statement arrived and parsed fine — what failed is
/// committing the transaction it produced. `1197` is also the one whose
/// documented meaning already matches the fix: MySQL's own text for it is
/// "increase this variable", and this server's honest equivalent — since
/// `docs/enterprise-readiness.md` blocker 5 explains why the ceiling itself
/// does not move — is "commit in smaller pieces", which is what MySQL
/// operators are told to do with `1197` in practice regardless.
///
/// The one MiB figure is not restated as a literal here: the message points
/// at `@@inlaysql_max_transaction_bytes`, which [`crate::session::Session`]
/// answers from [`inlaysql::max_transaction_bytes`] — the same formula
/// `CowBTree::commit` measures the record against — so a number a client
/// could act on cannot drift from the number that actually refuses it.
fn transaction_too_large(message: &str) -> MysqlError {
    MysqlError::new(
        1197,
        "HY000",
        format!(
            "{message}; every commit here is one write-ahead-log record and \
...
mod tests {
    ...
    /// The refusal an `ann-benchmarks` bulk load actually hit
    /// (`bench/README.md`): a single autocommit statement whose write set did
    /// not fit one write-ahead-log record. It used to fall through
    /// `classify_storage`'s generic arm and reach the client as `1030`, naming
    /// nothing; it now names the limit and the fix.
    #[test]
    fn a_transaction_too_big_to_commit_is_1197_not_1030() {
        let error = from_engine(&Error::Storage(
            "transaction does not fit the write-ahead log (2000000 > 1048576 bytes)".into(),
        ));
        assert_eq!(error.code, 1197, "ER_TRANS_CACHE_FULL");
        assert_eq!(error.sqlstate, "HY000");
        assert!(
            error.message.contains("inlaysql_max_transaction_bytes"),
            "no pointer to the reported limit: {}",
            error.message
        );
        assert!(
            error.message.contains("2000000") && error.message.contains("1048576"),
            "the engine's own byte counts were dropped: {}",
            error.message
        );
    }

    /// The other half of the same refusal: caught mid-transaction, before the
    /// statement that would have overflowed the record even runs
    /// (`Engine::ensure_transaction_fits`). Previously `1568`, the same flat
    /// code every other `Error::Transaction` state message got — indistinguishable
    /// from "no transaction open" to a client branching on the number.
    #[test]
    fn a_transaction_refused_mid_batch_is_also_1197() {
        let error = from_engine(&Error::Transaction(
            "transaction is too large for the write-ahead log; commit it and start a new one"
                .into(),
        ));
        assert_eq!(error.code, 1197, "ER_TRANS_CACHE_FULL");
        assert_eq!(error.sqlstate, "HY000");
    ...
    /// Every other shape of [`Error::Transaction`] is a state problem, not a
    /// size one, and must keep the code that was always distinct from
    /// `1030`/`1197` — otherwise "already open" and "too large to commit"
    /// become the same number to a client that only reads the code.
    #[test]
    fn an_ordinary_transaction_state_error_stays_1568() {
        let error = from_engine(&Error::Transaction("a transaction is already open".into()));
        assert_eq!(error.code, 1568);
